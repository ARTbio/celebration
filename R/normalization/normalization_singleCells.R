...
option_list <- list(
  ...
  optparse::make_option(c("-m", "--method"), type="character", help="Name of the method/package used for the normalization : scater or Seurat"),
  optparse::make_option("--min_mean", type="character", help="Minimum average count of genes to be used for normalization with scater"),
  optparse::make_option("--size_factor", type="logical", help="Using size factors based on deconvolution from cell pools (TRUE) or considering library sizes as size Factors (FALSE) ? NB: Mandatory if there is less than 100 cells in dataset, you must choose library sizes"),
  optparse::make_option(c("-o", "--output_matrix"), type = "character", help="Path to the filtered matrix")
)
...
if(ncol(data.counts) < 100 & args$size_factor == TRUE){
  stop("You need at least 100 cells in your dataset to use the deconvolution method.")
}
...
if (args$method == "Seurat") {
  ...
  sce <- Seurat::NormalizeData(sce, scale.factor = median(sce@meta.data$nUMI))
